...
int main(int argc, char* argv[]) { 
	...
	// Open binary file and check for consistency between current parameters and those used to create binary
	cerr << "Opening binary file and checking for consistency...\n";
	ifstream ifs;
	ifs.open(strBinaryFile.c_str(),ios::binary);
	if (ifs.fail()) SR::srerror("Problem opening binary file.");
	...
	if (ifs.fail()) SR::srerror("Problem loading up binary file.");
	ifs.close();
	cerr << "done.\n";
	...
	for (int i=0;i<objParamChanges.NoChanges();++i) {
		...
		int db1 = ukPars.GetIntValue("intMaxDelayTimesteps");
		int db2 = ukPars.GetIntValue("intMaxDailyEvents"); 
		int db3 = static_cast<int>(maxVaccinesPerday);
