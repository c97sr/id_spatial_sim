...
double kernNeighbourSetup(SR::ParameterSet &p, SR::Node* pt1, SR::Node* pt2, double offset) {
	static double* cc = p.GetPointer("Decay_Generate_Spatial_Neighbour");
	...
	static double* k_offset = p.GetPointer("Offset_Generate_Spatial_Neighbour");
...
double kernSpatialInfection(SR::ParameterSet &p, SR::Node* pt1, SR::Node* pt2, double offset) {
	...
	static double* k_offset = p.GetPointer("Offset_Transmit_Spatial");
...
void GatherUKPoxInfoA::RegisterEventAfterApplication(vector<SR::UntimedEvent>::iterator ptEv, int cb) {
	...
	intCurrentStackSize++;
	if (intCurrentStackSize==eventStack.size()) {
...
string GatherUKPoxInfoA::CurrentCumPrev() {
	ostringstream oss;
	for (int i=0;i<vecInc.size();++i) oss << vecInc[i] << "\t";
	oss << "\r";
	return oss.str();
};

void GatherUKPoxInfoA::ClearCumPrev() {
	for (int i=0;i<vecInc.size();++i) vecInc[i]=0;
};
...
vector<double>::iterator GatherUKPoxInfoA::AccessData(int ne, int nt, int nr) {
	...
	if (index >= processData.size()) SR::srerror("Index calc not correct.");
	return rtnIt + index;
};
...
void GatherUKPoxInfoA::SetFileBase(string s) {
	if (s.size()>maxstringlength) SR::srerror("String toio long in GatherUKPoxInfoA::SetFileBase");
...
bool GatherUKPoxInfoA::AddNodeLocationToSpatialStack(SR::Node *ptN) {
	...
	if (intCurrentSpatialStackSize==locationStack.size()) return false;
...
bool GatherUKPoxInfoA::EventStackFull() {
	if (intCurrentSpatialStackSize==locationStack.size()) return true;
	else return false;
};
...
void CalcBetasWithAttackRate(SR::ParameterSet &p, SR::GridHex& g, SR::KERNEL k) {
	...
	cerr << "Attack rate is " <<  current_ar << " with R0 of " << (current_r0+r0_spatial) << ".\n";
